...
namespace MSFTBandLib.Command
{
    ...
    public class CommandResponse : ICommandResponse
    {
        ...
        /// <summary>
        /// Received data byte sequences
        /// </summary>
        public List<byte[]> Data = new List<byte[]>();

        /// <summary>
        /// Response status bytes sequence length
        /// </summary>
        ...
        public byte[] GetData(int index = 0) => Data[index];
        ...
        public IByteStream GetByteStream(int index = 0) => new ByteStream(GetData(index));

        /// <summary>
        ///	Get whether the response has status bytes.
        /// </summary>
        /// <returns>bool</returns>
        public bool StatusReceived() => Status.Any(s => s != 0);
        ...
        /// <param name="bytes">bytes</param>
        /// <returns>byte[]</returns>
        public static byte[] GetResponseDataBytesStart(byte[] bytes) => bytes.Skip(RESPONSE_STATUS_LENGTH).ToArray();
        ...
        {
            var offset = (bytes.Length - RESPONSE_STATUS_LENGTH);
        ...
        /// <param name="bytes">bytes</param>
        /// <returns>byte[]</returns>
        public static byte[] GetResponseStatusBytesStart(byte[] bytes) => bytes.Take(RESPONSE_STATUS_LENGTH).ToArray();
        ...
        {
            var ints = bytes.Select(b => (int) b).ToArray();
        ...
        /// <param name="bytes">bytes</param>
        /// <returns>bool</returns>
        public static bool ResponseBytesContainData(byte[] bytes) => (bytes.Length > RESPONSE_STATUS_LENGTH);
        ...
        {
            var status = GetResponseStatusBytesStart(bytes);
        ...
        {
            var status = GetResponseStatusBytesEnd(bytes);
            return ResponseBytesAreStatus(status);
        }
    }
}